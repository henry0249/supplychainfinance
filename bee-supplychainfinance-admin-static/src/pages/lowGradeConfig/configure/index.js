...
export default class Index extends Component {
  ...
  save() {
    ...
    saveLowGradeConfig(params).then(res => {
      if (res.code === 0) {
        message.success(res.msg, 1.5, () => { router.replace('/lowGradeConfig') })
      }
      else {
      ...
      <div className={styles.container}>
        <div className={styles.crumb}>
          <Breadcrumb className={styles.breadcrumb}>
            <Breadcrumb.Item><Link to={'/lowGradeConfig'}>规则配置</Link></Breadcrumb.Item>
            <Breadcrumb.Item>{subjectName}</Breadcrumb.Item>
          </Breadcrumb>
          <div className={styles.title}>规则配置详情</div>
          <div className={styles.btnBox}>
            <Popconfirm title="退出后内容将不能保存" okText="确定" cancelText="取消" onConfirm={() => { router.push('/lowGradeConfig') }}><Button type="primary" size='large'>取 消</Button></Popconfirm>
