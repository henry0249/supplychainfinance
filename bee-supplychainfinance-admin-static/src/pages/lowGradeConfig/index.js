...
export default class Index extends Component {
  ...
  //配置
  toHandle(id) {//进入
    const nextUrl = id === undefined ? `/lowGradeConfig/configure` : `/lowGradeConfig/configure?id=${id}`;
    router.push(nextUrl)
  }
      ...
      <div className={styles.container}>
        ...
        <div className={styles.body}>
          <Form className={styles.top} layout='inline'>
            <FormItem label='标的物'>
              <Input style={{ width: 300 }} placeholder="请输入" value={subjectName} onChange={e => this.searchChange('subjectName', e.target.value)} />
            </FormItem>
            <FormItem label='业务类型'>
