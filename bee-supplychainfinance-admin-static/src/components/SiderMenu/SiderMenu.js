...
export default class SiderMenu extends PureComponent {
    ...
    return (
      ...
      >
        ...
        <span>
          ...
          {
            todo.map(todoItem => {
              if (todoItem.menuId === menuId && todoItem.number) {
                return <span key={todoItem.menuId} className={styles.tipsCircle}>{todoItem.number}</span>
              }
            })
          }
        </span>
  ...
  getSubMenuOrItem = item => {
    ...
    // if(item.name === "审批管理"){
    // todo.map(todoItem => {
    //   if (todoItem.number && todoItem.menuId !== 51 && todoItem.menuId !== 58) {
    //     show = true;
    //   }
    // });
    // }
            ...
            title={(
              <span>
                ...
                <span>
                  ...
                  {
                      todo.map(todoItem => {
                        if (todoItem.menuId === item.menuId && todoItem.number) {
                          return <span key={todoItem.menuId} className={styles.tipsCircle}>{todoItem.number}</span>
                        }
                      })
                    }
                  {/* {show ? <span className={styles.tipsSmallCircle} /> : null} */}
                </span>
              </span>
            )
            }
