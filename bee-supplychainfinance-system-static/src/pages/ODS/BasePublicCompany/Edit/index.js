  ...
  componentDidMount() {
    ...
    if (id !== undefined) {
      getDataById(id).then(res => {
        if (res.code === 0) {
          setFieldsValue({
            publicTime: moment(res.data.publicTime),
            companyName: res.data.companyName,
            companyId: res.data.companyId,
          })
        } else {
          message.error('查询国内上市企业失败：' + res.msg);
        }
      })
    }
  }
  ...
  handleSave = () => {
    ...
    validateFields((err, values) => {
      if (!err) {
        let params = {
          ...values,
          publicTime: moment(values.publicTime).format("YYYY-MM-DD hh:mm:ss")
        }
        ...
        saveData(params).then(res => {
          if (res.code === 0) {
            message.success('保存国内上市企业成功！');
            router.goBack();
          } else {
            message.error('保存国内上市企业失败:' + res.msg);
          }
        })
      }
    })
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <Breadcrumb>
            ...
            <Breadcrumb.Item><Link to="/ods/basePublicCompany/index">国内上市企业名单</Link></Breadcrumb.Item>
            ...
            <FormItem label="上市日期">
              {
                getFieldDecorator('publicTime', {
                  ...
                  <DatePicker format="YYYY-MM-DD hh:mm:ss" allowClear={false} style={{ width: 240 }} placeholder="请选择" />
                )
              }
            </FormItem>
