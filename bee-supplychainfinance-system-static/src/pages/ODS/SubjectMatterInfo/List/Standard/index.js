...
export default class Index extends Component {
  ...
  componentDidMount() {
    ...
    getSubjectMatterAttribute({ typeName: key, attributeType: 2 }).then(res => {
      if (res.code === 0) {
  ...
  changeName = (name) => {
    ...
    resetFields(['sourceAddress'])
    this.setState({ name })
  }

  changeNameAdd = (name_add) => {
    const { resetFields } = this.props.form;
    resetFields(['sourceAddress_add'])
    ...
    validateFields(['tast'], (err, values) => {
      if (!err) {
        if (!values.tast || !value) {
          return message.warning('请先选择标的物和来源')
        }
        let params = {
          tast: values.tast,
          sourceAddress: value
        ...
        getSpecificationList(params).then(res => {
          if (res.code === 0) {
            ...
            message.error('获取规格列表失败：' + res.msg);
          }
        })
      }
    })
  ...
  handleSave = (id, update) => {
    const { validateFields } = this.props.form;
    let _self = this;
    validateFields(update ? ['specification', 'sourceAddress'] : ['tast_add', 'sourceAddress_add', 'specification_add', 'sourceAddress'], (err, values) => {
      if (!err) {
        let params = {}
        if (update) {
          if (!specification) {
            return message.warning('请输入规格名字')
          }
          params = {
            id,
        ...
        saveSpecificationInfo(params).then(res => {
          if (res.code === 0) {
            message.success('保存规格成功');
            _self.handleQuery(values.sourceAddress)
          } else {
            message.error('保存规格失败：' + res.msg);
          }
        })
      }
    })
  ...
  handleDelte = (row) => {
    const { validateFields } = this.props.form;
    let _self = this;
    validateFields(['tast', 'sourceAddress'], (err, values) => {
      if (!err) {
        if (row.isUse) {
          return Modal.error({
            title: '删除标的物规格',
            content: '该规格已含有价格数据，无法删除！',
          });
        } else {
          Modal.confirm({
            title: '删除标的物规格',
            content: `删除标的物“${values.tast}”的规格“${row.specification}”吗？`,
            okText: '确认',
            cancelText: '取消',
            onOk: () => {
      ...
      render: (text, row, record) => editId !== null && editId === row.id ? <FormItem>
        {
          getFieldDecorator('specification', {
            ...
            <Input style={{ width: 240 }} placeholder="规格名称" />
          )
        }
      </FormItem> : text,
      width: '40%'
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          <Breadcrumb>
            ...
            <Breadcrumb.Item><Link to="/ods/subjectMatterInfo/list">标的列表</Link></Breadcrumb.Item>
          ...
          <Row type="flex" justify="space-between" align="middle">
            <Col>
              <span>操作：可编辑、添加和删除指定标的物的规格属性。</span>
            </Col>
            <Col>
              <Button onClick={() => this.setState({ visible: true })} icon="plus" type='primary' style={{ marginRight: 16 }}>添加新规格</Button>
            ...
            <FormItem label="标的物来源">
              {
                getFieldDecorator('sourceAddress_add', {
                  rules: [{ required: true, message: '请选择标的物来源' }]
                })(
                  <Select
                    style={{ width: 240 }}
                    placeholder="请选择标的物来源"
                  >
                    {
            ...
            <FormItem label="规格名称">
              {
                getFieldDecorator('specification_add', {
                  ...
                  <Input style={{ width: 240, }} placeholder="请输入规格名称" />
                )
              }
            </FormItem>
          </Form>
