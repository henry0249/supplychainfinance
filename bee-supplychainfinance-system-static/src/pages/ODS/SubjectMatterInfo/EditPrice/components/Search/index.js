import { Component } from 'react';
import { Form, Button, Breadcrumb, Card, Table, Row, Col, Input, InputNumber, Select, message, Modal } from 'antd';
import router from 'umi/router';
import Link from 'umi/link';
import withRouter from 'umi/withRouter';
import styles from './index.less';
import {
  getTypes,
  getSearch,
  getPriceDatas,
  savePriceInfo
} from '../../../service';

const FormItem = Form.Item;
const { Option } = Select;

@withRouter
@Form.create()
export default class Index extends Component {
  state = {
    key: null,
    tabs: [],
    current: 1,
    editId: null,
    name: null,
    names: [],
    details: {},
    data: [],
    priceUnit: '',
    page: {
      totalPage: 0,
      totalRecords: 0,
    }
  }

  componentDidMount() {
    const { key } = this.props.location.query;
    getTypes().then(res => {
      if (res.code === 0) {
        let tabs = [];
        res.data.forEach(i => {
          tabs.push({
            key: i.typeName,
            tab: `${i.typeName}（${i.subjectMatterCount}）`
          })
        })
        this.setState({
          key: key || tabs[0].key,
          tabs
        }, () => {
          this.getLists(key || tabs[0].key)
        })
      } else {
        message.error('获取标的物大类失败：' + res.msg);
      }
    })
  }

  getLists = (typeName) => {
    getSearch(typeName).then(res => {
      if (res.code === 0) {
        let names = [], details = {};
        res.data.forEach(item => {
          names.push(item.name);
          details[item.name] = item.detailDTOS
        })
        this.setState({
          names,
          details
        })
      } else {
        message.error('获取标的物筛选项失败：' + res.msg);
      }
    })
  }

  onTabChange = (key, type) => {
    const { resetFields } = this.props.form;
    resetFields(['name', 'id'])
    this.setState({
      [type]: key,
      current: 1,
      editId: null,
      name: null,
      names: [],
      details: {},
      data: [],
      priceUnit: ''
    }, () => this.getLists(key));
  };

  currentChange = (current) => {
    this.setState({
      current
    })
  }

  changeName = (name) => {
    const { resetFields } = this.props.form;
    resetFields(['id'])
    this.setState({ name })
  }

  handleSearch = () => {
    const { validateFields } = this.props.form;
    validateFields((err, values) => {
      if (!err) {
        if (values.id === undefined) {
          return Modal.error({
            title: '筛选价格',
            content: '请选择标的物名称，标的物特性后再进行筛选！',
          })
        }
        getPriceDatas(values.id).then(res => {
          if (res.code === 0) {
            this.setState({
              data: res.data.priceDTOList,
              priceUnit: res.data.priceUnit,
              page: {
                totalPage: Math.ceil(res.data.priceDTOList.length / 10),
                totalRecords: res.data.priceDTOList.length
              }
            })
          } else {
            message.error('获取标的物价格列表失败：' + res.msg);
          }
        })
      }
    })
  }

  handleSave = () => {
    const { validateFields } = this.props.form;
    const { editId } = this.state;
    validateFields((err, values) => {
      if (!err) {
        let params = {
          id: editId,
          priceManualInput: values.priceManualInput,
          price: values.price
        }
        savePriceInfo(params).then(res => {
          if (res.code === 0) {
            message.success('保存标的物价格成功');
            getPriceDatas(values.id).then(res => {
              if (res.code === 0) {
                this.setState({
                  editId: null,
                  data: res.data.priceDTOList,
                  priceUnit: res.data.priceUnit,
                  page: {
                    totalPage: Math.ceil(res.data.priceDTOList.length / 10),
                    totalRecords: res.data.priceDTOList.length
                  }
                })
              } else {
                message.error('获取标的物价格列表失败：' + res.msg);
              }
            })
          } else {
            message.error('保存标的物价格失败：' + res.msg);
          }
        })
      }
    })
  }

  render() {
    const { getFieldDecorator } = this.props.form;
    const { key, tabs, current, editId, name, names, details, data, priceUnit, page } = this.state;
    const columns = [
      {
        title: '时间',
        dataIndex: 'priceDate',
        key: 'priceDate',
        width: '20%'
      }, {
        title: '咨询价格',
        dataIndex: 'priceManualInput',
        key: 'priceManualInput',
        render: (text, row) => editId !== null && editId === row.id ? <FormItem>
          {
            getFieldDecorator('priceManualInput', {
              initialValue: text
            })(
              <InputNumber style={{ width: 80 }} placeholder="规格名称" />
            )
          }
          <span>{` ${priceUnit}`}</span>
        </FormItem> : text + ` ${priceUnit}`,
        width: '20%'
      }, {
        title: '三方价格',
        dataIndex: 'price',
        key: 'price',
        render: (text, row) => editId !== null && editId === row.id ? <FormItem>
          {
            getFieldDecorator('price', {
              initialValue: text
            })(
              <InputNumber style={{ width: 80 }} placeholder="规格名称" />
            )
          }
          <span> 元/吨</span>
        </FormItem> : text + ` ${priceUnit}`,
        width: '20%'
      }, {
        title: '操作',
        key: 'action',
        render: (text, row, record) => editId !== null && editId === row.id ?
          <div>
            <span onClick={this.handleSave.bind(this)} style={{ fontSize: 14, color: '#1890FF', cursor: 'pointer', marginRight: 12 }}>保存</span>
            <span onClick={() => this.setState({ editId: null })} style={{ fontSize: 14, color: '#1890FF', cursor: 'pointer' }}>取消</span>
          </div> :
          <div>
            <span onClick={() => this.setState({ editId: row.id })} style={{ fontSize: 14, color: '#1890FF', cursor: 'pointer' }}>编辑价格</span>
          </div>,
        width: '40%'
      }
    ];

    return (
      <Card
        className={styles.container}
        style={{ width: '100%' }}
        bordered={false}
        tabList={tabs}
        activeTabKey={key}
        onTabChange={key => {
          this.onTabChange(key, 'key');
        }}
      >
        <Form style={{ marginBottom: 24 }}>
          <Row>
            <Col span={6}>
              <FormItem label="标的物名称">
                {
                  getFieldDecorator('name')(
                    <Select
                      style={{ width: 180 }}
                      placeholder="请选择"
                      onChange={(value) => this.changeName(value)}
                    >
                      {
                        names.length !== 0 && names.map((item, i) => <Option value={item} key={i}>{item}</Option>)
                      }
                    </Select>
                  )
                }
              </FormItem>
            </Col>
            <Col span={10}>
              <FormItem label="标的物特性">
                {
                  getFieldDecorator('id')(
                    <Select style={{ width: 240 }} placeholder="请选择">
                      {
                        name && details[name].length !== 0 && details[name].map(item => <Option value={item.id} key={item.id}>{item.speSouStaInfo}</Option>)
                      }
                    </Select>
                  )
                }
              </FormItem>
            </Col>
            <Col style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', height: 40 }} span={8}>
              <Button onClick={this.handleSearch.bind(this)} type='primary'>筛选</Button>
            </Col>
          </Row>
        </Form>
        <Table
          rowKey="id"
          columns={columns}
          dataSource={data}
          pagination={{
            showQuickJumper: true,
            current: current,
            pageSize: 10,
            total: page.totalRecords,
            onChange: this.currentChange,
            showTotal: (total, range) => `共 ${page.totalRecords} 条记录 第 ${current} / ${page.totalPage} 页`,
          }}
        />
      </Card>
    )
  }
}
