  ...
  getData = (current, type) => {
    ...
    validateFields((err, values) => {
      if (!err) {
        ...
        if (type === 0) {
          params['roleType'] = values.roleType
          getDataByType(params).then(res => {
            if (res.code === 0) {
              ...
              message.error('获取核心企业白名单列表失败：' + res.msg);
            }
          })
        } else if (type === 1) {
    ...
    Modal.confirm({
      ...
      onOk() {
        deleteData({ id: row.id }).then(res => {
          if (res.code === 0) {
            message.success('删除企业"' + row.companyName + '"成功！');
            _self.getData(1, type)
          } else {
            message.error('删除企业失败：' + res.msg);
          }
        })
      },
      onCancel() { },
  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: (text, row, record) => <div>
          <span onClick={() => router.push(`/ods/baseCoreCompany/edit?id=${row.id}`)} style={{ fontSize: 14, color: '#1890FF', cursor: 'pointer', marginRight: 12 }}>编辑</span>
    ...
    return (
      <div className={styles.container}>
        <Card
          bordered={false}
          title="数据仓库：供应链平台核心企业白名单"
          extra={<Button onClick={() => router.push('/ods/baseCoreCompany/edit')} type="primary" icon="plus">添加新企业</Button>}
        >
          <Form layout="inline">
            <Row gutter={8}>
                ...
                <FormItem label="企业类型">
                  {
                    getFieldDecorator('roleType', {
                      ...
                      <Select allowClear={false} style={{ width: 150 }} placeholder="请选择">
                        ...
                        <Option value={1}>委托企业</Option>
                        <Option value={2}>担保公司</Option>
                        <Option value={3}>供货企业</Option>
                        <Option value={4}>购货企业</Option>
                        <Option value={5}>货代企业</Option>
                      </Select>
                    )
                  }
                </FormItem>
