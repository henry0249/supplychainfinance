...
import { getMacroInflationList, updateMacroInflation } from '../services/index'
...
export default class EditTable extends Component {
  ...
  getData(key = this.tabKey, pageSize = this.state.pageSize, currentPage = this.state.currentPage) {
    getMacroInflationList({ key, pageSize, currentPage }).then(res => {
      this.setState({
        list: res.data,
        ...res.page
      })
  ...
  save(id, row) {//保存编辑或者添加保存
    ...
    let { list } = this.state;
    params.id = id;
    updateMacroInflation(params, this.tabKey).then(res => {
      if (res.code === 0) {
