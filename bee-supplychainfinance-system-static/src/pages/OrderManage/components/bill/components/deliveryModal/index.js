...
export default class DeliveryModal extends Component {
  ...
  submit = () => {
    const { callback, orderBusinessId, refreshData, dispatch } = this.props;
    ...
    validateFields((err, data) => {
      ...
      if (data.draggerInvoice && data.draggerInvoice.fileList.length === 0) {
        return message.error('请上传至少一张放货证明');
      }
      ...
      dispatch({
        type: 'bill/submitDelivery',
        payload: params,
        success: () => {
          message.success('提交放货证明成功');
          refreshData && refreshData();
          this.setState({
            fileList: [],
            proofs: []
          }, () => callback && callback("visibleDelivery"))
        },
        error: (msg) => {
          message.error("提交放货证明失败：" + msg);
        }
      })
    })
  }
