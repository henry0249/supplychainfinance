...
export default class TransferModal extends Component {
  ...
  componentDidMount() {
    ...
    if (id && data) {
      Object.keys(data).forEach(key => {
        if (key === 'goodsPowerTransferProofDTO') {
          data[key].forEach(ele => {
            if (id === ele.goodsPowerBusinessId) {
              let filesArr = [];
  ...
  submit = () => {
    ...
    validateFields((err, data) => {
      ...
      if (files.length === 0) {
        return message.error('请上传至少一张货权转移证明');
      } else {
        files.forEach(item => {
          fileList.push({
            fileName: item.name,
      ...
      let params = {
        ...
        goodsAmount: data.goodsAmount, // 发货数量
      ...
      // 货权转移证明业务id 可传可不传
      if (id) {
        params['goodsPowerBusinessId'] = id;
      }

      dispatch({
        type: 'bill/submitTransfer',
        payload: params,
        success: () => {
          message.success('提交货权转移证明成功');
          refreshData && refreshData();
          this.setState({
            fileList: [],
            files: []
          }, () => callback && callback("visibleTransfer"))
        },
        error: (msg) => {
          message.error("提交货权转移证明失败：" + msg);
        }
      })
    })
  }
    ...
    return (
      <div className={styles.modal}>
        ...
        <Form className={"bill-modal-form " + styles.formBox}>
          <FormItem label='发货数量' className={styles.formItem}>
            {getFieldDecorator('goodsAmount', {
              rules: [{ required: true, message: '请输入发货数量' }]
            })(
              <InputNumber style={{ width: 200, marginRight: 10 }} placeholder="请输入" />
            )}
            <span>吨</span>
          </FormItem>
