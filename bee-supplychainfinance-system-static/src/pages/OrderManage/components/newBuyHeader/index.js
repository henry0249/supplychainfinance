...
export default class Index extends Component {
  ...
  handle = (type, letterType) => {
    const { dispatch, onShowModal, onChangeTab } = this.props;
      ...
      if (orderInfo.orderStatus === 0) {
        ...
        if (role.roleId === 4) {
          return <div className={styles.btnBox}>
            ...
            <Button type="primary" onClick={this.handle.bind(this, 1)}>编辑合同</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 0 ? true : false} onClick={this.handle.bind(this, 2, 0)}>提交委托采购函</Button>
          </div>
        } else if (role.roleId === 6 || role.roleId === 7 || role.roleId === 8) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" onClick={this.handle.bind(this, 3)}>审核合同</Button>
          </div>
        } else {
        ...
        if (role.roleId === 1) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 1 ? true : false} onClick={this.handle.bind(this, 4)}>提交保证金证明</Button>
          </div>
        } else if (role.roleId === 10 || role.roleId === 11 || role.roleId === 12) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 0 ? true : false} onClick={this.handle.bind(this, 5)}>审核保证金</Button>
          </div>
        } else {
          return <div className={styles.btnBox}></div>
        }
        ...
        if (role.roleId === 2) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 1 ? true : false} onClick={this.handle.bind(this, 6)}>提交货权转移证明</Button>
          </div>
        } else if (role.roleId === 10) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 1 ? true : false} onClick={this.handle.bind(this, 6)}>帮供应商提交货权转移证明</Button>
            <Button type="primary" onClick={this.handle.bind(this, 8)}>终止发货</Button>
          </div>
        } else if (role.roleId === 7) {
          return <div className={styles.btnBox}>
            <Button onClick={() => router.goBack()}>返 回</Button>
            <Button type="primary" disabled={orderInfo.submitStatus === 0 ? true : false} onClick={this.handle.bind(this, 7)}>确认货权转移</Button>
          </div>
        } else {
          return <div className={styles.btnBox}></div>
        }
