...
export default class Index extends Component {
        ...
        Object.keys(data).forEach(key => {
          if (Object.keys(data[key]).length !== 0) {
            ...
            if (sessionStorage.businessMode === '0') {
              url = `orderId=${orderId}&accountType=${data[key].accountType}&buyAccountsId=${data[key].buyAccountsId}`;
            } else if (sessionStorage.businessMode === '1') {
              url = `orderId=${orderId}&accountType=${data[key].accountType}&saleAccountsId=${data[key].saleAccountsId}`;
            } else {
              url = `orderId=${orderId}&accountType=${data[key].accountType}&storageAccountsId=${data[key].storageAccountsId}`;
            }
            dispatch({
  ...
  confirm = (accountType, accountsId) => {
    ...
    if (sessionStorage.businessMode === '0') {
      params['buyAccountsId'] = accountsId;
    } else if (sessionStorage.businessMode === '1') {
      params['saleAccountsId'] = accountsId;
    } else {
      params['storageAccountsId'] = accountsId;
    }
  ...
  handleJump = (accountType, accountsId) => {
    ...
    let orderId = orderInfo.buyOrdersId || orderInfo.storageOrdersId;
    if (pathArr[2] === 'details') {
      if (accountsId) {
        router.push(`/${pathArr[1]}/settlement/edit?accountType=${accountType}&orderId=${orderId}&accountsId=${accountsId}`)
      } else {
        router.push(`/${pathArr[1]}/settlement/edit?accountType=${accountType}&orderId=${orderId}`)
      }
    } else {
      if (accountsId) {
        router.push(`/${pathArr[1]}/${pathArr[2]}/edit?accountType=${accountType}&orderId=${orderId}&accountsId=${accountsId}`)
      } else {
        router.push(`/${pathArr[1]}/${pathArr[2]}/edit?accountType=${accountType}&orderId=${orderId}`)
      }
    }
  }
                                ...
                                sessionStorage.businessMode === '0' ? (
                                  role.roleId === 1 || role.roleId === 10 ?
                                    <div className={styles.btnBox}>
                                      <Button onClick={this.handleJump.bind(this, data[key].accountType, data[key].buyAccountsId)}>进入编辑</Button>
                                      <Button style={{ marginLeft: 10 }} type="primary" onClick={this.confirm.bind(this, data[key].accountType, data[key].buyAccountsId)}>确认结算</Button>
                                    </div>
                                    :
                                    null
                                ) :
                                  (
                                    role.roleId === 1 || role.roleId === 10 ?
                                      <div className={styles.btnBox}>
                                        <Button onClick={this.handleJump.bind(this, data[key].accountType, data[key].storageAccountsId)}>进入编辑</Button>
                                        <Button style={{ marginLeft: 10 }} type="primary" onClick={this.confirm.bind(this, data[key].accountType, data[key].storageAccountsId)}>确认结算</Button>
                                      </div>
                                      :
                                      null
                                  )
                      ...
                      <div className={styles.table}>
                        ...
                        </div>
                        {
                          data[key].collectionInvoicesAlready.map((item, index) =>
                            <div className={styles.row} key={"c_a_" + index}>
                        ...
                        </div>
                        {
                          data[key].collectionInvoicesNot.map((item, index) =>
                            <div className={styles.row} key={'c_n_' + index}>
                              <span></span>
                              <span>{item.explaining}</span>
                              <span>{item.collectionAmount}</span>
                              <span></span>
                      ...
                      <div className={styles.table}>
                        ...
                        </div>
                        {
                          data[key].productAccounts.map((item, index) =>
                            <div className={styles.row} key={'p_' + index}>
                        ...
                        }
                        {
                          data[key].productAccounts.map((item, index) => {
                            ...
                            if (index == data[key].productAccounts.length - 1) {
                              return (
                                <div className={styles.totle} key={"p_total_" + index}>
                                  <span>合计</span>
                      ...
                      <div className={styles.table}>
                        ...
                        </div>
                        {
                          data[key].qualityRequirements.map((item, index) =>
                            <div className={styles.row} key={'q_' + index}>
