...
export default class LetterModal extends Component {
  ...
  submit = () => {
    ...
    validateFields(['draggerInvoice', 'remark'], (err, data) => {
      if (err) {
        return false;
      }

      if (data.draggerInvoice && data.draggerInvoice.fileList.length === 0) {
        return message.error(`请上传至少一张委托付款函`);
      }
