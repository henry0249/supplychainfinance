...
import { Form, Button, Row, Col, Table, Card, Radio, Divider, message, Modal } from 'antd';
...
export default class Index extends Component {
  ...
  componentDidMount() {
    const id = this.props.location.query ? this.props.location.query.id
      ...
      : ''
    const orderId = this.props.location.query ? this.props.location.query.orderId
      ? this.props.location.query.orderId : ''
      : ''
    ...
    request(`/supplychainfinance-audit/earlyWariningRecord/getRecordDeatilById?id=${id}`,
      { method: 'get' }).then(res => {
        if (res.code === 0) {
          ...
          message.error('获取预警记录失败：' + res.msg)
        } else {
    ...
    //获取操作日志
    request(`/supplychainfinance-audit/earlyWariningRecord/getLogsByRecordId?currentPage=1&id=${id}&pageSize=10`,
      { method: 'get' }).then(res => {
        if (res && res.code === 0) {
          this.setState({
            ...res.page,
      ...
      if (!err) {
        ...
        if (warningStatus == 2) {
          request(`/supplychainfinance-audit/earlyWariningRecord/checkingWariningRecord`, {
            ...
            if (res && res.code === 0) {
              resetFields()
              this.fileComp.setState({ fileList: [] })//将文件上传组件里的上传列表置空
              this.setState({ file: {} })
              this.countDown()
              //刷新预警详情
              request(`/supplychainfinance-audit/earlyWariningRecord/getRecordDeatilById?id=${id}`,
  ...
  backApproval = () => {
    ...
    request(`/supplychainfinance-audit/earlyWariningRecord/cacelApplyWariningRecord?id=${id}&orderId=${orderId}`, {
      ...
      if (res && res.code === 0) {
        ...
        //获取操作日志
        request(`/supplychainfinance-audit/earlyWariningRecord/getLogsByRecordId?currentPage=${1}&id=${id}&pageSize=${pageSize}`,
          { method: 'get' }).then(res => {
            if (res && res.code === 0) {
              this.setState({
                ...res.page,
  ...
  render() {
    const { currentPage, pageSize, totalPage, totalRecords, warningDetail, logs, steps } = this.state;
          ...
          >
            <div style={{ minHeight: 463 }}>
              <div className={styles.headWrap}>
                <div className={styles.formHead}>业务名称：<span>{warningDetail.businessName}</span></div>
          ...
          <div className={styles.formwrapper} >
            ...
            >
              <div style={{ minHeight: 463 }}>
                ...
                <span>{warningDetail.nowOperateReason}</span>
              </div>
            </Card>
          </div>
      } else {//风控负责人对应页面
