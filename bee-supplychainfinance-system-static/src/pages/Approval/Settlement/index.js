...
const api = `${baseUrls.domain}/supplychainfinance-audit/AccountsExamine/applies`;
...
export default class Index extends Component {
  ...
  componentDidMount() {
    request(api + `?currentPage=1&pageSize=10`, {
      method: 'POST',
      // financeToken: 'finance-61945865-27e5-4fa7-a24b-3d4e8e2dc96b',
      body: {}
    }).then((res) => {
      if (res && res.code === 0) {
        this.setState({
  ...
  handleJump = (type, id, orderId) => {
    sessionStorage.setItem("businessMode", type);
    if (type === 4 || type === 1) {
      router.push(`/approval/settlement/details/childDetails?enterType=0&type=settlement&key=settlement&id=${id}&orderId=${orderId}`)
    } else {
      router.push(`/approval/settlement/details?enterType=0&type=settlement&key=settlement&id=${id}`)
    }
  }
  ...
  render() {
    ...
    const columns = [{
      ...
      render: (text, row, record) =>
        row.type === 1 || row.type === 4 ?
          <span onClick={this.handleJump.bind(this, row.type, row.orderId, row.motherOrderId)} style={{ color: '#40a9ff', cursor: 'pointer' }}>进入</span>
          :
          <span onClick={this.handleJump.bind(this, row.type, row.orderId)} style={{ color: '#40a9ff', cursor: 'pointer' }}>进入</span>,
      width: '20%'
    }];
    ...
    return (
      <div className={styles.container}>
        <div className={styles.crumb}>
          <Breadcrumb>
            <Breadcrumb.Item>结算审批管理</Breadcrumb.Item>
          </Breadcrumb>
        </div>
