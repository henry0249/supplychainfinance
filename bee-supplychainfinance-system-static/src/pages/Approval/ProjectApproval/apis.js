export default {
  ...
  getList: {
    //条件查询获取贷前立项审批列表 首页
    api: url =>
      `/supplychainfinance-input/commonProcessSchedule/getProcessListByCondition?${url}`,
    type: 'POST'
  },
  getReport: {
    //立项报告获取立项报告
  ...
  getScoreRisk: {
    ...
    api: param =>
      `/supplychainfinance-input/${getApplyController()}/getRiskScore`,
    type: 'POST'
  },
  updateRiskScore: {
    //立项申请详情(一二三风险评分/宏观经济指数/风险项信息) 更新数据
    api: param =>
      `/supplychainfinance-input/${getApplyController()}/updateRiskScore`,
    type: 'POST'
  },
...
//获得当前业务类型
const getType = () =>
  window.g_app._store.getState().projectApproval.businessMode
