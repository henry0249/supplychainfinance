...
const api = `${baseUrls.domain}/supplychainfinance-audit/audit/delay/applies`;
...
export default class Index extends Component {
  ...
  handleJump = (type, id) => {
    ...
    router.push(`/approval/deferred/details?enterType=0&type=deferred&key=bill&id=${id}`)
  }
  ...
  render() {
    ...
    const columns = [{
      ...
      render: (text, row, record) =>
        <span onClick={this.handleJump.bind(this, row.businessType, row.orderBusinessId)} style={{ color: '#40a9ff', cursor: 'pointer' }}>进入</span>,
      width: '20%'
    }];
    ...
    return (
      <div className={styles.container}>
        <div className={styles.crumb}>
          <Breadcrumb>
            <Breadcrumb.Item>延期提货审批</Breadcrumb.Item>
          </Breadcrumb>
        </div>
