...
export default class IncomingData extends Component {
  ...
  componentDidMount() {
    ...
    switch (type) {
      case 'p':
        this.props.dispatch({
          type: 'application/setBuyDetailData',
          payload: applyId,
      ...
      case 's':
        this.props.dispatch({
          type: 'application/setSaleDetailData',
          payload: applyId,
          callback: (code, msg, data) => {
            if (code === 0) {
              this.setState(
      ...
      case 'w':
        this.props.dispatch({
          type: 'application/setStorageDetailData',
          payload: applyId,
          callback: (code, msg, data) => {
            if (code === 0) {
              this.setState(
      ...
      case 'L':
        this.props.dispatch({
          type: 'application/setLargeBuyDetailData',
          payload: applyId,
          callback: (code, msg, data) => {
            if (code === 0) {
              this.setState(
  ...
  render() {
    ...
    return (
      <div className={styles.IncomingData}>
        ...
        {showTwo && type == 's' && (
          <Card
            title={<div style={{ fontWeight: 600 }}>{'委托企业经营信息'}</div>}
            bordered={false}
            style={{ marginTop: 20 }}
          >
            <div>
