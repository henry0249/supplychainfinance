...
@Form.create()
@connect(({ global, application, loading, incoming }) => ({
  incoming,
  application,
...
export default class Index extends Component {
  ...
  toHandle(id, type, status) {
    const { dispatch } = this.props
    dispatch({
      type: 'application/pickBusinessMode',
      payload: type,
      callback: () => {
        router.push(`/application/handle?id=${id}&status=${status}`)
      }
    })
  }

  render() {
    const { list, counts } = this.props.application
    ...
    const columns = [{
      ...
      render(text, row) {
        if (row.belongNow === 1) {
          return <div><Badge status="error" offset={[4, -2]}>{text}</Badge></div>
        }
        return text
      },
      width: '20%'
      ...
      title: '操作',
      key: 'action',
      render: (text, row, record) => <a onClick={this.toHandle.bind(this, row.id, row.businessMode, row.applyStatus)}>进入</a>,
      width: '12%'
    }]
