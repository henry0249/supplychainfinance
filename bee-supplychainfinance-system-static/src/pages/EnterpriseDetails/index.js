...
export default class Index extends Component {
  ...
  componentDidMount() {
    getEnterprisePermission().then(res => {
      if (res.code === 0) {
        this.setState({
          roles: res.data
        })
  ...
  handleUser = (row) => {
    getAllUsers(row.roleId).then(res => {
      if (res.code === 0) {
        this.setState({
          rolesModal: row,
          userVisible: true,
  ...
  render() {
    const { user, role } = this.props.global;
    ...
    const columns = [
      {
        ...
        render: (text, row, record) =>
          <div>
            {
              row.enterpriseId === 0 ?
                <span style={{ color: 'rgba(0,0,0,0.25)', cursor: 'not-allowed' }}>配置</span>
                :
            ...
            {
              row.enterpriseId === 0 ?
                <span style={{ color: 'rgba(0,0,0,0.25)', cursor: 'not-allowed' }}>删除</span>
                :
        ...
        <div className={styles.header}>
          <Row>
            <Col className={styles.col}>
              ...
              <span className={styles.name}>{user.companyName || '未知'}</span>
            </Col>
          </Row>
        </div>
                  ...
                  <FormItem label="权限组名称">
                    {
                      getFieldDecorator('roleName', {
                        initialValue: rolesModal.roleName || null,
                        rules: [
                          { required: true, message: '请输入权限组名称' }
                        ]
                      })(
